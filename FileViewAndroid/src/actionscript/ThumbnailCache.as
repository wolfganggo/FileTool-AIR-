package actionscript
{
	...
	public class ThumbnailCache
	{
		...
		public function saveThumbnail (obj:Bitmap, origFs:File, thumbCounter:int):void
		{
			if (thumbFolder_ == null || !thumbFolder_.exists || pictFolder_.length == 0) {
				return;
			}
			var image_w:int = obj.bitmapData.width;
			var image_h:int = obj.bitmapData.height;
			...
			var byteArray:ByteArray = obj.bitmapData.getPixels (new Rectangle (0, 0, image_w, image_h));
			byteArray.compress();
		...
		public function saveThumbnails (path:String, name:String, obj:Bitmap, files:Array, newFolder:Boolean):void
		{
			if (cacheFs_ == null || !cacheFs_.exists) {
				return;
			}
			...
			for (var ix:int = 0; ix < 1; ix++) {
				var cache:Object = new Object;
				cache.kOriginalPath = path;
				var origFile:File = files[ix] as File;
				cache.kFileDate = origFile.modificationDate.toLocaleString();
				
				try {
					var file1:File = cachefs.resolvePath (ix.toString() + "a");
					var fstr1:FileStream = new FileStream();
					fstr1.open (file1, FileMode.WRITE);
					fstr1.writeObject (cache);
					fstr1.close();
					var file2:File = cachefs.resolvePath (ix.toString());
					var fstr2:FileStream = new FileStream();
					fstr2.open (file2, FileMode.WRITE);
					//fstr2.writeObject (arr[ix]);
					fstr2.writeObject (obj);
					fstr2.close();
				}
				catch (error:Error) {
				}
